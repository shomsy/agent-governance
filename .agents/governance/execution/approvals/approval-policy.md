...
# Build commands (examples — adapt to project ecosystem)
ALLOW: <package-manager> run build
ALLOW: <test-runner>
ALLOW: <linter>
...
3. Rules are prefix-matched: `ALLOW: <package-manager> run` approves
   `<package-manager> run build`, `<package-manager> run test`, etc.
...
Operation: <package-manager> install <dependency>@latest
Risk Level: Medium
Rationale: Required for external data serialization.
Alternatives: Could implement serialization natively.
