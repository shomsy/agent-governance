---
title: system-tools-poly-internal-secretsops-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Tools Poly Internal Secretsops How This Works

## What this folder is

`system/tools/poly/internal/secretsops/` is one internal Poly tooling slice.
...
- this folder owns one concrete tooling step, often starting with `manage_secrets.go`
...
flowchart TD
    ...
    A["1. a real `poly ...`, gate, or script entry reaches this tooling slice"]:::step1 --> B["2. this folder owns one concrete tooling step, often starting with `manage_secrets.go`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as PreparePromotedSecrets
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/tools/poly/internal/secretsops/` through the current story
    Owned->>Owned: Step 2: call `PreparePromotedSecrets(...)` to do the main folder-owned work
...
- **Step 2:** The first real work starts in `manage_secrets.go` through `PreparePromotedSecrets(...)`.
...
- when the `system/tools/poly/internal/secretsops/` story needs this responsibility, it opens `manage_secrets.go`
...
sequenceDiagram
    ...
    participant File as manage_secrets.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `manage_secrets.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `manage_secrets.go` because this file owns the next small responsibility.
...
- start with `PreparePromotedSecrets(...)` in `manage_secrets.go` when that action looks wrong

## What to remember

- `system/tools/poly/internal/secretsops/` exists so this slice has one obvious home.
