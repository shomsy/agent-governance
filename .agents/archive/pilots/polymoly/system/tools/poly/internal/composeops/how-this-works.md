---
title: system-tools-poly-internal-composeops-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Tools Poly Internal Composeops How This Works

## What this folder is

`system/tools/poly/internal/composeops/` is one internal Poly tooling slice.
...
- this folder owns one concrete tooling step, often starting with `compose_specs.go`
...
flowchart TD
    ...
    A["1. a real `poly ...`, gate, or script entry reaches this tooling slice"]:::step1 --> B["2. this folder owns one concrete tooling step, often starting with `compose_specs.go`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as Base
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/tools/poly/internal/composeops/` through the current story
    Owned->>Owned: Step 2: call `Base(...)` to do the main folder-owned work
...
- **Step 2:** The first real work starts in `compose_specs.go` through `Base(...)`.
...
- its main action is still visible in the code, starting with `Base(...)`

When the story opens this file:

- when the `system/tools/poly/internal/composeops/` story needs this responsibility, it opens `compose_specs.go`
...
- start with `Base(...)` in `compose_specs.go` when that action looks wrong

## What to remember

- `system/tools/poly/internal/composeops/` exists so this slice has one obvious home.
