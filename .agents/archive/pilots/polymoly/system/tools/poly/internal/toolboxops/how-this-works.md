---
title: system-tools-poly-internal-toolboxops-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Tools Poly Internal Toolboxops How This Works

## What this folder is

`system/tools/poly/internal/toolboxops/` is one internal Poly tooling slice.
...
- this folder owns one concrete tooling step, often starting with `release_toolbox.go`
...
flowchart TD
    ...
    A["1. a real `poly ...`, gate, or script entry reaches this tooling slice"]:::step1 --> B["2. this folder owns one concrete tooling step, often starting with `release_toolbox.go`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as ParseInt
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/tools/poly/internal/toolboxops/` through the current story
    Owned->>Owned: Step 2: call `ParseInt(...)` to do the main folder-owned work
...
- **Step 2:** The first real work starts in `release_toolbox.go` through `ParseInt(...)`.
...
- its main action is still visible in the code, starting with `ParseInt(...)`

When the story opens this file:

- when the `system/tools/poly/internal/toolboxops/` story needs this responsibility, it opens `release_toolbox.go`
...
- the result of `ParseInt(...)` for the next caller
- a concrete return value, file write, check result, or summary depending on the path

Why you open it first:

- open this file when the symptom points to `ParseInt(...)` doing the wrong thing

```mermaid
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as release_toolbox.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `release_toolbox.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `release_toolbox.go` because this file owns the next small responsibility.
...
- start with `ParseInt(...)` in `release_toolbox.go` when that action looks wrong

## What to remember

- `system/tools/poly/internal/toolboxops/` exists so this slice has one obvious home.
