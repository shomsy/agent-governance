---
title: system-tools-poly-internal-renderer-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Tools Poly Internal Renderer How This Works

## What this folder is

`system/tools/poly/internal/renderer/` is one internal Poly tooling slice.
...
- this folder owns one concrete tooling step, often starting with `runner_summary.go`
...
flowchart TD
    ...
    A["1. a real `poly ...`, gate, or script entry reaches this tooling slice"]:::step1 --> B["2. this folder owns one concrete tooling step, often starting with `runner_summary.go`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as WriteRunnerSummary
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/tools/poly/internal/renderer/` through the current story
    Owned->>Owned: Step 2: call `WriteRunnerSummary(...)` to do the main folder-owned work
...
- **Step 2:** The first real work starts in `runner_summary.go` through `WriteRunnerSummary(...)`.
...
- when the `system/tools/poly/internal/renderer/` story needs this responsibility, it opens `runner_summary.go`
...
sequenceDiagram
    ...
    participant File as runner_summary.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `runner_summary.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `runner_summary.go` because this file owns the next small responsibility.
...
- start with `WriteRunnerSummary(...)` in `runner_summary.go` when that action looks wrong

## What to remember

- `system/tools/poly/internal/renderer/` exists so this slice has one obvious home.
