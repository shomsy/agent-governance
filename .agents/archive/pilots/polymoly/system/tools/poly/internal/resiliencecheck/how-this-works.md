---
title: system-tools-poly-internal-resiliencecheck-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Tools Poly Internal Resiliencecheck How This Works

## What this folder is

`system/tools/poly/internal/resiliencecheck/` is one internal Poly tooling slice.
...
- this folder owns one concrete tooling step, often starting with `check_system_resilience.go`
...
flowchart TD
    ...
    A["1. a real `poly ...`, gate, or script entry reaches this tooling slice"]:::step1 --> B["2. this folder owns one concrete tooling step, often starting with `check_system_resilience.go`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as CheckIncidentReplay
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/tools/poly/internal/resiliencecheck/` through the current story
    Owned->>Owned: Step 2: call `CheckIncidentReplay(...)` to do the main folder-owned work
...
- **Step 2:** The first real work starts in `check_system_resilience.go` through `CheckIncidentReplay(...)`.
- **Step 3:** From here, the story moves to one smaller file, child slice, or boundary that can do the next concrete job.
- **Step 4:** At the end, the caller has something concrete to carry forward: a file on disk, a rendered asset, a proof artifact, or a clear next state.

## Direct files in this folder

### `check_system_resilience.go`
...
- when the `system/tools/poly/internal/resiliencecheck/` story needs this responsibility, it opens `check_system_resilience.go`
...
sequenceDiagram
    ...
    participant File as check_system_resilience.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `check_system_resilience.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `check_system_resilience.go` because this file owns the next small responsibility.
...
- start with `CheckIncidentReplay(...)` in `check_system_resilience.go` when that action looks wrong

## What to remember

- `system/tools/poly/internal/resiliencecheck/` exists so this slice has one obvious home.
