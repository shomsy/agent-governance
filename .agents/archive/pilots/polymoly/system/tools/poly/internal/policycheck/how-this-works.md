---
title: system-tools-poly-internal-policycheck-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Tools Poly Internal Policycheck How This Works

## What this folder is

`system/tools/poly/internal/policycheck/` is one internal Poly tooling slice.
...
- this folder owns one concrete tooling step, often starting with `check_policy_compliance.go`
...
flowchart TD
    ...
    A["1. a real `poly ...`, gate, or script entry reaches this tooling slice"]:::step1 --> B["2. this folder owns one concrete tooling step, often starting with `check_policy_compliance.go`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as CheckOPACompliance
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/tools/poly/internal/policycheck/` through the current story
    Owned->>Owned: Step 2: call `CheckOPACompliance(...)` to do the main folder-owned work
...
- **Step 2:** The first real work starts in `check_policy_compliance.go` through `CheckOPACompliance(...)`.
- **Step 3:** From here, the story moves to one smaller file, child slice, or boundary that can do the next concrete job.
- **Step 4:** At the end, the caller has something concrete to carry forward: a file on disk, a rendered asset, a proof artifact, or a clear next state.

## Direct files in this folder

### `check_policy_compliance.go`

This file is one direct stop in the story for this folder.

Why this name is honest:

- its main action is still visible in the code, starting with `CheckOPACompliance(...)`

When the story opens this file:

- when the `system/tools/poly/internal/policycheck/` story needs this responsibility, it opens `check_policy_compliance.go`
...
- the result of `CheckOPACompliance(...)` for the next caller
- a concrete return value, file write, check result, or summary depending on the path

Why you open it first:

- open this file when the symptom points to `CheckOPACompliance(...)` doing the wrong thing

```mermaid
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as check_policy_compliance.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `check_policy_compliance.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `check_policy_compliance.go` because this file owns the next small responsibility.
...
- start with `CheckOPACompliance(...)` in `check_policy_compliance.go` when that action looks wrong

## What to remember

- `system/tools/poly/internal/policycheck/` exists so this slice has one obvious home.
