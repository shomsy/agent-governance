---
title: system-tools-poly-internal-policy-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Tools Poly Internal Policy How This Works

## What this folder is

`system/tools/poly/internal/policy/` is one internal Poly tooling slice.
...
- this folder owns one concrete tooling step, often starting with `governance_and_gitops.go`
...
flowchart TD
    ...
    A["1. a real `poly ...`, gate, or script entry reaches this tooling slice"]:::step1 --> B["2. this folder owns one concrete tooling step, often starting with `governance_and_gitops.go`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as CheckCIProfileContract
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/tools/poly/internal/policy/` through the current story
    Owned->>Owned: Step 2: call `CheckCIProfileContract(...)` to do the main folder-owned work
...
- **Step 2:** The first real work starts in `governance_and_gitops.go` through `CheckCIProfileContract(...)`.
...
- its main action is still visible in the code, starting with `TestLockGitOpsRepo(...)`

When the story opens this file:

- when the `system/tools/poly/internal/policy/` story needs this responsibility, it opens `gitops_test.go`
...
### `governance_and_gitops.go`

This file is one direct stop in the story for this folder.

Why this name is honest:

- its main action is still visible in the code, starting with `CheckCIProfileContract(...)`

When the story opens this file:

- when the `system/tools/poly/internal/policy/` story needs this responsibility, it opens `governance_and_gitops.go`
...
- the result of `CheckCIProfileContract(...)` for the next caller
- a concrete return value, file write, check result, or summary depending on the path

Why you open it first:

- open this file when the symptom points to `CheckCIProfileContract(...)` doing the wrong thing

```mermaid
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as governance_and_gitops.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `governance_and_gitops.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `governance_and_gitops.go` because this file owns the next small responsibility.
- **Step 2:** The file does its own narrow action instead of mixing it into a bigger caller.
- **Step 3:** The next caller gets a concrete result, not another vague promise.
...
- start with `TestLockGitOpsRepo(...)` in `gitops_test.go` when that action looks wrong
- start with `CheckCIProfileContract(...)` in `governance_and_gitops.go` when that action looks wrong

## What to remember

- `system/tools/poly/internal/policy/` exists so this slice has one obvious home.
