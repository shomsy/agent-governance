---
title: system-runtime-networking-kubernetes-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Networking Kubernetes How This Works

## What this folder is

`system/runtime/networking/kubernetes/` ships networking-side runtime assets.
...
- this folder contributes concrete assets or defaults, often starting with `core-infra-namespace.yaml`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `core-infra-namespace.yaml`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as core-infra-namespace.yaml
    participant Next as apps/
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/networking/kubernetes/` through the current story
    Owned->>Owned: Step 2: perform the folder-owned work in `core-infra-namespace.yaml`
...
- **Step 2:** The first real work starts in `core-infra-namespace.yaml`.
- **Step 3:** From here, the story moves to one smaller file, child slice, or boundary that can do the next concrete job.
- **Step 4:** At the end, the caller has something concrete to carry forward: a file on disk, a rendered asset, a proof artifact, or a clear next state.

## Direct files in this folder

### `core-infra-namespace.yaml`

This file ships the `core-infra-namespace.yaml` config, policy, or data asset that the next technical step reads directly.
...
- when the `system/runtime/networking/kubernetes/` story needs this responsibility, it opens `core-infra-namespace.yaml`
...
- the shipped `core-infra-namespace.yaml` asset
...
sequenceDiagram
    ...
    participant File as core-infra-namespace.yaml
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `core-infra-namespace.yaml`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `core-infra-namespace.yaml` because this file owns the next small responsibility.
...
Open [`apps/how-this-works.md`](./apps/how-this-works.md).
...
Open [`infra/how-this-works.md`](./infra/how-this-works.md).
...
- start with `core-infra-namespace.yaml` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/networking/kubernetes/` exists so this slice has one obvious home.
