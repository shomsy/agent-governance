---
title: system-runtime-capabilities-multi-tenant-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities Multi Tenant How This Works

## What this folder is

`system/runtime/capabilities/multi-tenant/` is one shipped runtime capability slice.
...
- this folder contributes concrete assets or defaults, often starting with `tenants-example.yaml`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `tenants-example.yaml`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as tenants-example.yaml
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/multi-tenant/` through the current story
    Owned->>Owned: Step 2: perform the folder-owned work in `tenants-example.yaml`
...
- **Step 2:** The first real work starts in `tenants-example.yaml`.
...
This file ships the `tenants-example.yaml` config, policy, or data asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/multi-tenant/` story needs this responsibility, it opens `tenants-example.yaml`
...
- the shipped `tenants-example.yaml` asset
...
sequenceDiagram
    ...
    participant File as tenants-example.yaml
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `tenants-example.yaml`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `tenants-example.yaml` because this file owns the next small responsibility.
...
- start with `tenants-example.yaml` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/multi-tenant/` exists so this slice has one obvious home.
