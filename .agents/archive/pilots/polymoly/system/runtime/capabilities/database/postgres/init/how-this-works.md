---
title: system-runtime-capabilities-database-postgres-init-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities Database Postgres Init How This Works

## What this folder is

`system/runtime/capabilities/database/postgres/init/` holds shipped initialization assets for this capability.
...
- this folder contributes concrete assets or defaults, often starting with `init.sql`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `init.sql`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as init.sql
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/database/postgres/init/` through the current story
    Owned->>Owned: Step 2: perform the folder-owned work in `init.sql`
...
- **Step 2:** The first real work starts in `init.sql`.
...
This file ships the `init.sql` config, policy, or data asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/database/postgres/init/` story needs this responsibility, it opens `init.sql`
...
- start with `init.sql` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/database/postgres/init/` exists so this slice has one obvious home.
