---
title: system-runtime-capabilities-database-mysql-pooling-proxysql-init-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities Database Mysql Pooling Proxysql Init How This Works

## What this folder is

`system/runtime/capabilities/database/mysql/pooling/proxysql/init/` holds shipped initialization assets for this capability.

These files matter when the selected capability needs first-start data, schema, or bootstrap steps.
...
- this folder contributes concrete assets or defaults, often starting with `proxysql-init.sql`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `proxysql-init.sql`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as proxysql-init.sql
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/database/mysql/pooling/proxysql/init/` through the current story
    Owned->>Owned: Step 2: perform the folder-owned work in `proxysql-init.sql`
...
- **Step 2:** The first real work starts in `proxysql-init.sql`.
...
This file ships the `proxysql-init.sql` config, policy, or data asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/database/mysql/pooling/proxysql/init/` story needs this responsibility, it opens `proxysql-init.sql`
...
- the shipped `proxysql-init.sql` asset
...
sequenceDiagram
    ...
    participant File as proxysql-init.sql
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `proxysql-init.sql`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `proxysql-init.sql` because this file owns the next small responsibility.
...
- start with `proxysql-init.sql` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/database/mysql/pooling/proxysql/init/` exists so this slice has one obvious home.
