---
title: system-runtime-capabilities-cache-varnish-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities Cache Varnish How This Works

## What this folder is

`system/runtime/capabilities/cache/varnish/` is one shipped runtime capability slice.
...
- this folder contributes concrete assets or defaults, often starting with `default.vcl`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `default.vcl`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as default.vcl
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/cache/varnish/` through the current story
    Owned->>Owned: Step 2: perform the folder-owned work in `default.vcl`
...
- **Step 2:** The first real work starts in `default.vcl`.
...
This file ships the `default.vcl` asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/cache/varnish/` story needs this responsibility, it opens `default.vcl`
...
- start with `default.vcl` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/cache/varnish/` exists so this slice has one obvious home.
