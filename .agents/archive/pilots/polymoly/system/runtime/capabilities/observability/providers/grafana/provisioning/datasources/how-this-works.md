---
title: system-runtime-capabilities-observability-providers-grafana-provisioning-datasources-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities Observability Providers Grafana Provisioning Datasources How This Works

## What this folder is

`system/runtime/capabilities/observability/providers/grafana/provisioning/datasources/` is one shipped runtime capability slice.
...
- this folder contributes concrete assets or defaults, often starting with `loki.yaml`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `loki.yaml`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as loki.yaml
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/observability/providers/grafana/provisioning/datasources/` through the current story
    Owned->>Owned: Step 2: perform the folder-owned work in `loki.yaml`
...
- **Step 2:** The first real work starts in `loki.yaml`.
...
This file ships the `loki.yaml` config, policy, or data asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/observability/providers/grafana/provisioning/datasources/` story needs this responsibility, it opens `loki.yaml`
...
- when the `system/runtime/capabilities/observability/providers/grafana/provisioning/datasources/` story needs this responsibility, it opens `prometheus.yaml`
...
- start with `loki.yaml` when the shipped asset or contract itself looks wrong
- start with `prometheus.yaml` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/observability/providers/grafana/provisioning/datasources/` exists so this slice has one obvious home.
