---
title: system-runtime-capabilities-app-php-roadrunner-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities App Php Roadrunner How This Works

## What this folder is

`system/runtime/capabilities/app/php/roadrunner/` is one shipped runtime capability slice.
...
- this folder contributes concrete assets or defaults, often starting with `.rr.yaml`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `.rr.yaml`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as .rr.yaml
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/app/php/roadrunner/` through the current story
    Owned->>Owned: Step 2: perform the folder-owned work in `.rr.yaml`
...
- **Step 2:** The first real work starts in `.rr.yaml`.
...
This file ships the `.rr.yaml` config, policy, or data asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/app/php/roadrunner/` story needs this responsibility, it opens `.rr.yaml`
...
- when the `system/runtime/capabilities/app/php/roadrunner/` story needs this responsibility, it opens `Dockerfile`
...
- start with `.rr.yaml` when the shipped asset or contract itself looks wrong
- start with `Dockerfile` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/app/php/roadrunner/` exists so this slice has one obvious home.
