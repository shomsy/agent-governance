---
title: system-runtime-capabilities-app-node-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities App Node How This Works

## What this folder is

`system/runtime/capabilities/app/node/` is one shipped runtime capability slice.
...
sequenceDiagram
    ...
    participant Next as engine/
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/app/node/` through the current story
...
- when the `system/runtime/capabilities/app/node/` story needs this responsibility, it opens `.dockerignore`
...
This file ships the `healthcheck.mjs` web or script asset that a later runtime or UI step reads directly.
...
- when the `system/runtime/capabilities/app/node/` story needs this responsibility, it opens `healthcheck.mjs`
...
- the shipped `healthcheck.mjs` asset
- a concrete file the next render or runtime step can read directly
...
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as healthcheck.mjs
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `healthcheck.mjs`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `healthcheck.mjs` because this file owns the next small responsibility.
- **Step 2:** The file does its own narrow action instead of mixing it into a bigger caller.
- **Step 3:** The next caller gets a concrete result, not another vague promise.
...
- when the `system/runtime/capabilities/app/node/` story needs this responsibility, it opens `module.yaml`
...
- start with `healthcheck.mjs` when the shipped asset or contract itself looks wrong
- start with `module.yaml` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/app/node/` exists so this slice has one obvious home.
