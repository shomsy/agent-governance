---
title: system-runtime-capabilities-profile-law-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities Profile Law How This Works

## What this folder is

`system/runtime/capabilities/profile-law/` is one shipped runtime capability slice.
...
- this folder contributes concrete assets or defaults, often starting with `enterprise.yaml`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `enterprise.yaml`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as enterprise.yaml
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/profile-law/` through the current story
    Owned->>Owned: Step 2: perform the folder-owned work in `enterprise.yaml`
...
- **Step 2:** The first real work starts in `enterprise.yaml`.
...
This file ships the `enterprise.yaml` config, policy, or data asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/profile-law/` story needs this responsibility, it opens `enterprise.yaml`
...
- the shipped `enterprise.yaml` asset
...
This file ships the `localhost.yaml` config, policy, or data asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/profile-law/` story needs this responsibility, it opens `localhost.yaml`
...
- the shipped `localhost.yaml` asset
- a concrete file the next render or runtime step can read directly
...
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as localhost.yaml
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `localhost.yaml`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `localhost.yaml` because this file owns the next small responsibility.
- **Step 2:** The file does its own narrow action instead of mixing it into a bigger caller.
- **Step 3:** The next caller gets a concrete result, not another vague promise.
...
This file ships the `minimal.yaml` config, policy, or data asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/profile-law/` story needs this responsibility, it opens `minimal.yaml`
...
- the shipped `minimal.yaml` asset
- a concrete file the next render or runtime step can read directly
...
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as minimal.yaml
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `minimal.yaml`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `minimal.yaml` because this file owns the next small responsibility.
- **Step 2:** The file does its own narrow action instead of mixing it into a bigger caller.
- **Step 3:** The next caller gets a concrete result, not another vague promise.
...
- when the `system/runtime/capabilities/profile-law/` story needs this responsibility, it opens `production.yaml`
...
- start with `enterprise.yaml` when the shipped asset or contract itself looks wrong
- start with `localhost.yaml` when the shipped asset or contract itself looks wrong
- start with `minimal.yaml` when the shipped asset or contract itself looks wrong
- start with `production.yaml` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/profile-law/` exists so this slice has one obvious home.
