---
title: system-runtime-capabilities-security-zero-trust-pki-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities Security Zero Trust Pki How This Works

## What this folder is

`system/runtime/capabilities/security/zero-trust/pki/` is one shipped runtime capability slice.
...
- this folder contributes concrete assets or defaults, often starting with `internal_ca.crt`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `internal_ca.crt`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as internal_ca.crt
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/security/zero-trust/pki/` through the current story
    Owned->>Owned: Step 2: perform the folder-owned work in `internal_ca.crt`
...
- **Step 2:** The first real work starts in `internal_ca.crt`.
...
This file ships the `internal_ca.crt` asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/security/zero-trust/pki/` story needs this responsibility, it opens `internal_ca.crt`
...
- the shipped `internal_ca.crt` asset
...
- start with `internal_ca.crt` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/security/zero-trust/pki/` exists so this slice has one obvious home.
