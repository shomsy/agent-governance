---
title: system-runtime-capabilities-security-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities Security How This Works

## What this folder is

`system/runtime/capabilities/security/` is one shipped runtime capability slice.
...
- this folder contributes concrete assets or defaults, often starting with `threat-model.json`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `threat-model.json`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as threat-model.json
    participant Next as policies/
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/security/` through the current story
    Owned->>Owned: Step 2: perform the folder-owned work in `threat-model.json`
...
- **Step 2:** The first real work starts in `threat-model.json`.
...
This file ships the `threat-model.json` config, policy, or data asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/security/` story needs this responsibility, it opens `threat-model.json`
...
- the shipped `threat-model.json` asset
...
Open [`policies/how-this-works.md`](./policies/how-this-works.md).
...
Open [`seccomp/how-this-works.md`](./seccomp/how-this-works.md).
...
Open [`vault/how-this-works.md`](./vault/how-this-works.md).
...
Open [`zero-trust/how-this-works.md`](./zero-trust/how-this-works.md).
...
- start with `threat-model.json` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/security/` exists so this slice has one obvious home.
