---
title: system-runtime-capabilities-security-vault-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities Security Vault How This Works

## What this folder is

`system/runtime/capabilities/security/vault/` is one shipped runtime capability slice.
...
- this folder contributes concrete assets or defaults, often starting with `RUNBOOK.md`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `RUNBOOK.md`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as RUNBOOK.md
    participant Next as config/
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/security/vault/` through the current story
    Owned->>Owned: Step 2: perform the folder-owned work in `RUNBOOK.md`
...
- **Step 2:** The first real work starts in `RUNBOOK.md`.
...
This file ships the `RUNBOOK.md` asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/security/vault/` story needs this responsibility, it opens `RUNBOOK.md`
...
- start with `RUNBOOK.md` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/security/vault/` exists so this slice has one obvious home.
