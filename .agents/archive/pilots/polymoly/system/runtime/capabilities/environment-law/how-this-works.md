---
title: system-runtime-capabilities-environment-law-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities Environment Law How This Works

## What this folder is

`system/runtime/capabilities/environment-law/` is one shipped runtime capability slice.
...
- this folder contributes concrete assets or defaults, often starting with `local.yaml`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `local.yaml`"]:::step2
...
sequenceDiagram
    ...
    Entry->>Owned: Step 1: reach `system/runtime/capabilities/environment-law/` through the current story
...
- when the `system/runtime/capabilities/environment-law/` story needs this responsibility, it opens `local.yaml`
...
This file ships the `prod.yaml` config, policy, or data asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/environment-law/` story needs this responsibility, it opens `prod.yaml`
...
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as prod.yaml
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `prod.yaml`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `prod.yaml` because this file owns the next small responsibility.
- **Step 2:** The file does its own narrow action instead of mixing it into a bigger caller.
- **Step 3:** The next caller gets a concrete result, not another vague promise.
...
- when the `system/runtime/capabilities/environment-law/` story needs this responsibility, it opens `stage.yaml`
...
- start with `prod.yaml` when the shipped asset or contract itself looks wrong
- start with `stage.yaml` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/environment-law/` exists so this slice has one obvious home.
