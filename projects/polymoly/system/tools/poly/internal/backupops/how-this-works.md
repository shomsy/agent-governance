---
title: system-tools-poly-internal-backupops-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Tools Poly Internal Backupops How This Works

## What this folder is

`system/tools/poly/internal/backupops/` is one internal Poly tooling slice.
...
- this folder owns one concrete tooling step, often starting with `backup_recovery.go`
...
flowchart TD
    ...
    A["1. a real `poly ...`, gate, or script entry reaches this tooling slice"]:::step1 --> B["2. this folder owns one concrete tooling step, often starting with `backup_recovery.go`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as CheckMinIOBackupPolicy
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/tools/poly/internal/backupops/` through the current story
    Owned->>Owned: Step 2: call `CheckMinIOBackupPolicy(...)` to do the main folder-owned work
...
- **Step 2:** The first real work starts in `backup_recovery.go` through `CheckMinIOBackupPolicy(...)`.
...
- its main action is still visible in the code, starting with `CheckMinIOBackupPolicy(...)`

When the story opens this file:

- when the `system/tools/poly/internal/backupops/` story needs this responsibility, it opens `backup_recovery.go`
...
- the result of `CheckMinIOBackupPolicy(...)` for the next caller
- a concrete return value, file write, check result, or summary depending on the path

Why you open it first:

- open this file when the symptom points to `CheckMinIOBackupPolicy(...)` doing the wrong thing

```mermaid
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as backup_recovery.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `backup_recovery.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `backup_recovery.go` because this file owns the next small responsibility.
...
- start with `CheckMinIOBackupPolicy(...)` in `backup_recovery.go` when that action looks wrong

## What to remember

- `system/tools/poly/internal/backupops/` exists so this slice has one obvious home.
