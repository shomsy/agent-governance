---
title: system-tools-poly-internal-contracts-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Tools Poly Internal Contracts How This Works

## What this folder is

`system/tools/poly/internal/contracts/` is one internal Poly tooling slice.
...
- this folder owns one concrete tooling step, often starting with `command_contracts.go`
...
flowchart TD
    ...
    A["1. a real `poly ...`, gate, or script entry reaches this tooling slice"]:::step1 --> B["2. this folder owns one concrete tooling step, often starting with `command_contracts.go`"]:::step2
...
sequenceDiagram
    ...
    Entry->>Owned: Step 1: reach `system/tools/poly/internal/contracts/` through the current story
...
- **Step 2:** The first real work starts in `command_contracts.go` through `ApplyCommandSpec(...)`.
- **Step 3:** From here, the story moves to one smaller file, child slice, or boundary that can do the next concrete job.
- **Step 4:** At the end, the caller has something concrete to carry forward: a file on disk, a rendered asset, a proof artifact, or a clear next state.

## Direct files in this folder

### `command_contracts.go`
...
- when the `system/tools/poly/internal/contracts/` story needs this responsibility, it opens `command_contracts.go`
...
sequenceDiagram
    ...
    participant File as command_contracts.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `command_contracts.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `command_contracts.go` because this file owns the next small responsibility.
...
- its main action is still visible in the code, starting with `TestExecuteEnforcesTimeout(...)`

When the story opens this file:

- when the `system/tools/poly/internal/contracts/` story needs this responsibility, it opens `command_test.go`
...
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as command_test.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `command_test.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `command_test.go` because this file owns the next small responsibility.
- **Step 2:** The file does its own narrow action instead of mixing it into a bigger caller.
- **Step 3:** The next caller gets a concrete result, not another vague promise.

Important functions:

- `TestExecuteEnforcesTimeout(...)`
...
- its main action is still visible in the code, starting with `TestComposeCommandLocal(...)`

When the story opens this file:

- when the `system/tools/poly/internal/contracts/` story needs this responsibility, it opens `compose_test.go`
...
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as compose_test.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `compose_test.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `compose_test.go` because this file owns the next small responsibility.
- **Step 2:** The file does its own narrow action instead of mixing it into a bigger caller.
- **Step 3:** The next caller gets a concrete result, not another vague promise.

Important functions:

- `TestComposeCommandLocal(...)`
  One proof case in this file. It locks one expected behavior so a regression fails loudly.
- `TestComposeCommandRejectsUnsupportedEnvironment(...)`
  One proof case in this file. It locks one expected behavior so a regression fails loudly.
- `TestLocalBootstrapComposeCommandTargetsMinimalBootstrapSurface(...)`
  One proof case in this file. It locks one expected behavior so a regression fails loudly.
...
- start with `ApplyCommandSpec(...)` in `command_contracts.go` when that action looks wrong
- start with `TestExecuteEnforcesTimeout(...)` in `command_test.go` when that action looks wrong
- start with `TestComposeCommandLocal(...)` in `compose_test.go` when that action looks wrong

## What to remember

- `system/tools/poly/internal/contracts/` exists so this slice has one obvious home.
