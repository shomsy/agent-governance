---
title: system-tools-poly-internal-system-engine-apply-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Tools Poly Internal System Engine Apply How This Works

## What this folder is

`system/tools/poly/internal/system/engine/apply/` is one internal Poly tooling slice.
...
- this folder owns one concrete tooling step, often starting with `command_specs.go`
...
flowchart TD
    ...
    A["1. a real `poly ...`, gate, or script entry reaches this tooling slice"]:::step1 --> B["2. this folder owns one concrete tooling step, often starting with `command_specs.go`"]:::step2
...
sequenceDiagram
    ...
    Entry->>Owned: Step 1: reach `system/tools/poly/internal/system/engine/apply/` through the current story
...
- **Step 2:** The first real work starts in `command_specs.go` through `ApplyCommandSpec(...)`.
...
- when the `system/tools/poly/internal/system/engine/apply/` story needs this responsibility, it opens `command_specs.go`
...
sequenceDiagram
    ...
    participant File as command_specs.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `command_specs.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `command_specs.go` because this file owns the next small responsibility.
...
- start with `ApplyCommandSpec(...)` in `command_specs.go` when that action looks wrong

## What to remember

- `system/tools/poly/internal/system/engine/apply/` exists so this slice has one obvious home.
