---
title: system-tools-poly-internal-dashboardops-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Tools Poly Internal Dashboardops How This Works

## What this folder is

`system/tools/poly/internal/dashboardops/` is one internal Poly tooling slice.
...
- this folder owns one concrete tooling step, often starting with `dashboard_snapshot.go`
...
flowchart TD
    ...
    A["1. a real `poly ...`, gate, or script entry reaches this tooling slice"]:::step1 --> B["2. this folder owns one concrete tooling step, often starting with `dashboard_snapshot.go`"]:::step2
...
sequenceDiagram
    ...
    Entry->>Owned: Step 1: reach `system/tools/poly/internal/dashboardops/` through the current story
...
- **Step 2:** The first real work starts in `dashboard_snapshot.go` through `WriteSnapshot(...)`.
- **Step 3:** From here, the story moves to one smaller file, child slice, or boundary that can do the next concrete job.
- **Step 4:** At the end, the caller has something concrete to carry forward: a file on disk, a rendered asset, a proof artifact, or a clear next state.

## Direct files in this folder

### `manage_system_dashboards_test.go`

This test file locks one real behavior in this folder and fails loudly when that behavior drifts.

Why this name is honest:

- its main action is still visible in the code, starting with `TestWriteSnapshotCreatesHTMLAndJSON(...)`

When the story opens this file:

- when the `system/tools/poly/internal/dashboardops/` story needs this responsibility, it opens `manage_system_dashboards_test.go`
...
sequenceDiagram
    ...
    participant File as manage_system_dashboards_test.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `manage_system_dashboards_test.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `manage_system_dashboards_test.go` because this file owns the next small responsibility.
- **Step 2:** The file does its own narrow action instead of mixing it into a bigger caller.
- **Step 3:** The next caller gets a concrete result, not another vague promise.

Important functions:

- `TestWriteSnapshotCreatesHTMLAndJSON(...)`
  One proof case in this file. It locks one expected behavior so a regression fails loudly.
- `TestRenderDoctorChecksPrioritizesFailures(...)`
  One proof case in this file. It locks one expected behavior so a regression fails loudly.
...
- when the `system/tools/poly/internal/dashboardops/` story needs this responsibility, it opens `dashboard_snapshot.go`
...
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as dashboard_snapshot.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `dashboard_snapshot.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `dashboard_snapshot.go` because this file owns the next small responsibility.
- **Step 2:** The file does its own narrow action instead of mixing it into a bigger caller.
- **Step 3:** The next caller gets a concrete result, not another vague promise.
...
- start with `TestWriteSnapshotCreatesHTMLAndJSON(...)` in `manage_system_dashboards_test.go` when that action looks wrong
- start with `WriteSnapshot(...)` in `dashboard_snapshot.go` when that action looks wrong

## What to remember

- `system/tools/poly/internal/dashboardops/` exists so this slice has one obvious home.
