---
title: system-tools-poly-internal-doctor-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Tools Poly Internal Doctor How This Works

## What this folder is

`system/tools/poly/internal/doctor/` is one internal Poly tooling slice.
...
- this folder owns one concrete tooling step, often starting with `diagnose_system_integrity.go`
...
flowchart TD
    ...
    A["1. a real `poly ...`, gate, or script entry reaches this tooling slice"]:::step1 --> B["2. this folder owns one concrete tooling step, often starting with `diagnose_system_integrity.go`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as CheckRequiredToolchain
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/tools/poly/internal/doctor/` through the current story
    Owned->>Owned: Step 2: call `CheckRequiredToolchain(...)` to do the main folder-owned work
...
- **Step 2:** The first real work starts in `diagnose_system_integrity.go` through `CheckRequiredToolchain(...)`.
- **Step 3:** From here, the story moves to one smaller file, child slice, or boundary that can do the next concrete job.
- **Step 4:** At the end, the caller has something concrete to carry forward: a file on disk, a rendered asset, a proof artifact, or a clear next state.

## Direct files in this folder

### `diagnose_system_integrity.go`

This file is one direct stop in the story for this folder.

Why this name is honest:

- its main action is still visible in the code, starting with `CheckRequiredToolchain(...)`

When the story opens this file:

- when the `system/tools/poly/internal/doctor/` story needs this responsibility, it opens `diagnose_system_integrity.go`
...
- the result of `CheckRequiredToolchain(...)` for the next caller
- a concrete return value, file write, check result, or summary depending on the path

Why you open it first:

- open this file when the symptom points to `CheckRequiredToolchain(...)` doing the wrong thing

```mermaid
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as diagnose_system_integrity.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `diagnose_system_integrity.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `diagnose_system_integrity.go` because this file owns the next small responsibility.
...
### `diagnose_system_integrity_test.go`

This test file locks one real behavior in this folder and fails loudly when that behavior drifts.

Why this name is honest:

- its main action is still visible in the code, starting with `TestWriteReportMarksGateFailAndCountsMissingChecks(...)`

When the story opens this file:

- when the `system/tools/poly/internal/doctor/` story needs this responsibility, it opens `diagnose_system_integrity_test.go`
...
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as diagnose_system_integrity_test.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `diagnose_system_integrity_test.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `diagnose_system_integrity_test.go` because this file owns the next small responsibility.
- **Step 2:** The file does its own narrow action instead of mixing it into a bigger caller.
- **Step 3:** The next caller gets a concrete result, not another vague promise.

Important functions:

- `TestWriteReportMarksGateFailAndCountsMissingChecks(...)`
...
- start with `CheckRequiredToolchain(...)` in `diagnose_system_integrity.go` when that action looks wrong
- start with `TestWriteReportMarksGateFailAndCountsMissingChecks(...)` in `diagnose_system_integrity_test.go` when that action looks wrong

## What to remember

- `system/tools/poly/internal/doctor/` exists so this slice has one obvious home.
