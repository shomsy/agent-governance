---
title: system-tools-poly-internal-platformops-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Tools Poly Internal Platformops How This Works

## What this folder is

`system/tools/poly/internal/platformops/` is one internal Poly tooling slice.
...
- this folder owns one concrete tooling step, often starting with `bootstrap_platform_environment.go`
...
flowchart TD
    ...
    A["1. a real `poly ...`, gate, or script entry reaches this tooling slice"]:::step1 --> B["2. this folder owns one concrete tooling step, often starting with `bootstrap_platform_environment.go`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as Bootstrap
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/tools/poly/internal/platformops/` through the current story
    Owned->>Owned: Step 2: call `Bootstrap(...)` to do the main folder-owned work
...
- **Step 2:** The first real work starts in `bootstrap_platform_environment.go` through `Bootstrap(...)`.
- **Step 3:** From here, the story moves to one smaller file, child slice, or boundary that can do the next concrete job.
- **Step 4:** At the end, the caller has something concrete to carry forward: a file on disk, a rendered asset, a proof artifact, or a clear next state.

## Direct files in this folder

### `bootstrap_platform_environment.go`

This file is one direct stop in the story for this folder.

Why this name is honest:

- its main action is still visible in the code, starting with `Bootstrap(...)`

When the story opens this file:

- when the `system/tools/poly/internal/platformops/` story needs this responsibility, it opens `bootstrap_platform_environment.go`
...
- the result of `Bootstrap(...)` for the next caller
- a concrete return value, file write, check result, or summary depending on the path

Why you open it first:

- open this file when the symptom points to `Bootstrap(...)` doing the wrong thing

```mermaid
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as bootstrap_platform_environment.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `bootstrap_platform_environment.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `bootstrap_platform_environment.go` because this file owns the next small responsibility.
...
- `bootstrapComposeSpec(...)`
  Small helper for one narrow sub-step. It exists so the main path stays readable.
- `contractsLocalBootstrapServices(...)`
  Small helper for one narrow sub-step. It exists so the main path stays readable.
- `HostsEntries(...)`
  Small helper for one narrow sub-step. It exists so the main path stays readable.
- `UpdateHostsEntries(...)`
...
### `provision_tenant_isolation.go`

This file is one direct stop in the story for this folder.

Why this name is honest:

- its main action is still visible in the code, starting with `TenantProvision(...)`

When the story opens this file:

- when the `system/tools/poly/internal/platformops/` story needs this responsibility, it opens `provision_tenant_isolation.go`
...
- the result of `TenantProvision(...)` for the next caller
- a concrete return value, file write, check result, or summary depending on the path

Why you open it first:

- open this file when the symptom points to `TenantProvision(...)` doing the wrong thing

```mermaid
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as provision_tenant_isolation.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `provision_tenant_isolation.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `provision_tenant_isolation.go` because this file owns the next small responsibility.
- **Step 2:** The file does its own narrow action instead of mixing it into a bigger caller.
- **Step 3:** The next caller gets a concrete result, not another vague promise.
...
- start with `Bootstrap(...)` in `bootstrap_platform_environment.go` when that action looks wrong
- start with `TenantProvision(...)` in `provision_tenant_isolation.go` when that action looks wrong

## What to remember

- `system/tools/poly/internal/platformops/` exists so this slice has one obvious home.
