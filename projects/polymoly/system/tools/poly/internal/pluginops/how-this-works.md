---
title: system-tools-poly-internal-pluginops-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Tools Poly Internal Pluginops How This Works

## What this folder is

`system/tools/poly/internal/pluginops/` is one internal Poly tooling slice.
...
- this folder owns one concrete tooling step, often starting with `plugin_management.go`
...
flowchart TD
    ...
    A["1. a real `poly ...`, gate, or script entry reaches this tooling slice"]:::step1 --> B["2. this folder owns one concrete tooling step, often starting with `plugin_management.go`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as Install
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/tools/poly/internal/pluginops/` through the current story
    Owned->>Owned: Step 2: call `Install(...)` to do the main folder-owned work
...
- **Step 2:** The first real work starts in `plugin_management.go` through `Install(...)`.
- **Step 3:** From here, the story moves to one smaller file, child slice, or boundary that can do the next concrete job.
- **Step 4:** At the end, the caller has something concrete to carry forward: a file on disk, a rendered asset, a proof artifact, or a clear next state.

## Direct files in this folder

### `orchestrate_plugin_lifecycle_test.go`

This test file locks one real behavior in this folder and fails loudly when that behavior drifts.

Why this name is honest:

- its main action is still visible in the code, starting with `TestCompareVersions(...)`

When the story opens this file:

- when the `system/tools/poly/internal/pluginops/` story needs this responsibility, it opens `orchestrate_plugin_lifecycle_test.go`
...
sequenceDiagram
    ...
    participant File as orchestrate_plugin_lifecycle_test.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `orchestrate_plugin_lifecycle_test.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `orchestrate_plugin_lifecycle_test.go` because this file owns the next small responsibility.
...
- `TestInstallAndFindInstalledByCommand(...)`
  One proof case in this file. It locks one expected behavior so a regression fails loudly.
- `TestValidateTrustRejectsUnsignedByDefault(...)`
  One proof case in this file. It locks one expected behavior so a regression fails loudly.
- `TestResolveFromRegistryHighestVersion(...)`
  One proof case in this file. It locks one expected behavior so a regression fails loudly.
- `TestValidateTrustResolvesPathExecutableDigest(...)`
  One proof case in this file. It locks one expected behavior so a regression fails loudly.
- `TestValidateTrustFailsClosedWhenDigestExecutableCannotBeResolved(...)`
  One proof case in this file. It locks one expected behavior so a regression fails loudly.

### `plugin_management.go`

This file is one direct stop in the story for this folder.

Why this name is honest:

- its main action is still visible in the code, starting with `Install(...)`

When the story opens this file:

- when the `system/tools/poly/internal/pluginops/` story needs this responsibility, it opens `plugin_management.go`
...
- the result of `Install(...)` for the next caller
- a concrete return value, file write, check result, or summary depending on the path

Why you open it first:

- open this file when the symptom points to `Install(...)` doing the wrong thing

```mermaid
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as plugin_management.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `plugin_management.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `plugin_management.go` because this file owns the next small responsibility.
- **Step 2:** The file does its own narrow action instead of mixing it into a bigger caller.
- **Step 3:** The next caller gets a concrete result, not another vague promise.
...
- `Remove(...)`
  Small helper for one narrow sub-step. It exists so the main path stays readable.
- `FindInstalledByCommand(...)`
  Small helper for one narrow sub-step. It exists so the main path stays readable.
- `NormalizeDescriptor(...)`
  Small helper for one narrow sub-step. It exists so the main path stays readable.
- `ValidateTrust(...)`
  Small helper for one narrow sub-step. It exists so the main path stays readable.
- `ResolveExecutableForTrust(...)`
  Small helper for one narrow sub-step. It exists so the main path stays readable.
- `ResolveExecutable(...)`
  Small helper for one narrow sub-step. It exists so the main path stays readable.
- `CompareVersions(...)`
...
- start with `TestCompareVersions(...)` in `orchestrate_plugin_lifecycle_test.go` when that action looks wrong
- start with `Install(...)` in `plugin_management.go` when that action looks wrong

## What to remember

- `system/tools/poly/internal/pluginops/` exists so this slice has one obvious home.
