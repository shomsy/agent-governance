---
title: system-tools-poly-internal-checkkit-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Tools Poly Internal Checkkit How This Works

## What this folder is

`system/tools/poly/internal/checkkit/` is one internal Poly tooling slice.
...
- this folder owns one concrete tooling step, often starting with `command_tools.go`
...
flowchart TD
    ...
    A["1. a real `poly ...`, gate, or script entry reaches this tooling slice"]:::step1 --> B["2. this folder owns one concrete tooling step, often starting with `command_tools.go`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as CaptureCommand
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/tools/poly/internal/checkkit/` through the current story
    Owned->>Owned: Step 2: call `CaptureCommand(...)` to do the main folder-owned work
...
- **Step 2:** The first real work starts in `command_tools.go` through `CaptureCommand(...)`.
...
- its main action is still visible in the code, starting with `TestResolveTimeoutDefault(...)`

When the story opens this file:

- when the `system/tools/poly/internal/checkkit/` story needs this responsibility, it opens `exec_test.go`
...
- when the `system/tools/poly/internal/checkkit/` story needs this responsibility, it opens `command_tools.go`
...
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as command_tools.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `command_tools.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `command_tools.go` because this file owns the next small responsibility.
- **Step 2:** The file does its own narrow action instead of mixing it into a bigger caller.
- **Step 3:** The next caller gets a concrete result, not another vague promise.
...
- its main action is still visible in the code, starting with `TestWriteRequired(...)`

When the story opens this file:

- when the `system/tools/poly/internal/checkkit/` story needs this responsibility, it opens `report_test.go`
...
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as report_test.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `report_test.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `report_test.go` because this file owns the next small responsibility.
- **Step 2:** The file does its own narrow action instead of mixing it into a bigger caller.
- **Step 3:** The next caller gets a concrete result, not another vague promise.
...
- `TestWriteRequired(...)`
  One proof case in this file. It locks one expected behavior so a regression fails loudly.
- `TestWriteRequiredFailsWhenParentMissing(...)`
  One proof case in this file. It locks one expected behavior so a regression fails loudly.
...
- start with `TestResolveTimeoutDefault(...)` in `exec_test.go` when that action looks wrong
- start with `CaptureCommand(...)` in `command_tools.go` when that action looks wrong
- start with `TestWriteRequired(...)` in `report_test.go` when that action looks wrong

## What to remember

- `system/tools/poly/internal/checkkit/` exists so this slice has one obvious home.
