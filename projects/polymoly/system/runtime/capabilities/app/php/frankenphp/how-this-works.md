---
title: system-runtime-capabilities-app-php-frankenphp-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities App Php Frankenphp How This Works

## What this folder is

`system/runtime/capabilities/app/php/frankenphp/` is one shipped runtime capability slice.
...
- this folder contributes concrete assets or defaults, often starting with `Caddyfile`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `Caddyfile`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as Caddyfile
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/app/php/frankenphp/` through the current story
    Owned->>Owned: Step 2: perform the folder-owned work in `Caddyfile`
...
- **Step 2:** The first real work starts in `Caddyfile`.
...
This file ships the `Caddyfile` asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/app/php/frankenphp/` story needs this responsibility, it opens `Caddyfile`
...
- when the `system/runtime/capabilities/app/php/frankenphp/` story needs this responsibility, it opens `Dockerfile`
...
- start with `Caddyfile` when the shipped asset or contract itself looks wrong
- start with `Dockerfile` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/app/php/frankenphp/` exists so this slice has one obvious home.
