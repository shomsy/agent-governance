---
title: system-runtime-capabilities-app-node-engine-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities App Node Engine How This Works

## What this folder is

`system/runtime/capabilities/app/node/engine/` holds the shipped engine or image build assets for this runtime capability.
...
sequenceDiagram
    ...
    Entry->>Owned: Step 1: reach `system/runtime/capabilities/app/node/engine/` through the current story
...
- when the `system/runtime/capabilities/app/node/engine/` story needs this responsibility, it opens `Dockerfile`
...
This file ships the `Dockerfile.wolfi` build recipe the next render or image step reads directly.
...
- when the `system/runtime/capabilities/app/node/engine/` story needs this responsibility, it opens `Dockerfile.wolfi`
...
- the shipped `Dockerfile.wolfi` asset
- a concrete file the next render or runtime step can read directly
...
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as Dockerfile.wolfi
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `Dockerfile.wolfi`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `Dockerfile.wolfi` because this file owns the next small responsibility.
- **Step 2:** The file does its own narrow action instead of mixing it into a bigger caller.
- **Step 3:** The next caller gets a concrete result, not another vague promise.
...
- start with `Dockerfile.wolfi` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/app/node/engine/` exists so this slice has one obvious home.
