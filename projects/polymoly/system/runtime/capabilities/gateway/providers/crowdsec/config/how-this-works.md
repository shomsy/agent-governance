---
title: system-runtime-capabilities-gateway-providers-crowdsec-config-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities Gateway Providers Crowdsec Config How This Works

## What this folder is

`system/runtime/capabilities/gateway/providers/crowdsec/config/` holds shipped configuration assets for this capability.
...
- this folder contributes concrete assets or defaults, often starting with `acquis.yaml`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `acquis.yaml`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as acquis.yaml
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/gateway/providers/crowdsec/config/` through the current story
    Owned->>Owned: Step 2: perform the folder-owned work in `acquis.yaml`
...
- **Step 2:** The first real work starts in `acquis.yaml`.
...
This file ships the `acquis.yaml` config, policy, or data asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/gateway/providers/crowdsec/config/` story needs this responsibility, it opens `acquis.yaml`
...
- start with `acquis.yaml` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/gateway/providers/crowdsec/config/` exists so this slice has one obvious home.
