---
title: system-runtime-capabilities-gateway-providers-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities Gateway Providers How This Works

## What this folder is

`system/runtime/capabilities/gateway/providers/` groups provider-specific implementations for the current capability.
...
- this folder contributes concrete assets or defaults, often starting with `crowdsec/`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `crowdsec/`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as crowdsec/
    participant Next as NarrowerFlow
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/gateway/providers/` through the current story
    Owned->>Owned: Step 2: hand the story into `crowdsec/` because that child slice owns the first concrete step
    Owned->>Next: Step 3: let `crowdsec/` continue the concrete file and function work
    Next-->>Result: Step 4: make the next visible summary, artifact, or state available
```

- **Step 1:** This is the moment the story actually enters this folder instead of staying in a higher router or parent helper.
- **Step 2:** The first real work starts in `crowdsec/`.
...
Open [`crowdsec/how-this-works.md`](./crowdsec/how-this-works.md).
...
Open [`traefik/how-this-works.md`](./traefik/how-this-works.md).
...
- open `crowdsec/how-this-works.md` when the symptom clearly belongs to that child story
- open `traefik/how-this-works.md` when the symptom clearly belongs to that child story

## What to remember

- `system/runtime/capabilities/gateway/providers/` exists so this slice has one obvious home.
