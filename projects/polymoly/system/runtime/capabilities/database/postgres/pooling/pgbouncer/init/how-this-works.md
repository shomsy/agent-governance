---
title: system-runtime-capabilities-database-postgres-pooling-pgbouncer-init-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities Database Postgres Pooling Pgbouncer Init How This Works

## What this folder is

`system/runtime/capabilities/database/postgres/pooling/pgbouncer/init/` holds shipped initialization assets for this capability.
...
- this folder contributes concrete assets or defaults, often starting with `userlist.txt`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `userlist.txt`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as userlist.txt
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/database/postgres/pooling/pgbouncer/init/` through the current story
    Owned->>Owned: Step 2: perform the folder-owned work in `userlist.txt`
...
- **Step 2:** The first real work starts in `userlist.txt`.
...
This file ships the `userlist.txt` asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/database/postgres/pooling/pgbouncer/init/` story needs this responsibility, it opens `userlist.txt`
...
- the shipped `userlist.txt` asset
...
- start with `userlist.txt` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/database/postgres/pooling/pgbouncer/init/` exists so this slice has one obvious home.
