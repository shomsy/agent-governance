---
title: system-runtime-capabilities-database-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities Database How This Works

## What this folder is

`system/runtime/capabilities/database/` is one shipped runtime capability slice.
...
- this folder contributes concrete assets or defaults, often starting with `dual-db-boundaries.json`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `dual-db-boundaries.json`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as dual-db-boundaries.json
    participant Next as contract/
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/database/` through the current story
    Owned->>Owned: Step 2: perform the folder-owned work in `dual-db-boundaries.json`
...
- **Step 2:** The first real work starts in `dual-db-boundaries.json`.
- **Step 3:** From here, the story moves to one smaller file, child slice, or boundary that can do the next concrete job.
- **Step 4:** At the end, the caller has something concrete to carry forward: a file on disk, a rendered asset, a proof artifact, or a clear next state.

## Direct files in this folder

### `dual-db-boundaries.json`

This file ships the `dual-db-boundaries.json` config, policy, or data asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/database/` story needs this responsibility, it opens `dual-db-boundaries.json`
...
- the shipped `dual-db-boundaries.json` asset
...
Open [`contract/how-this-works.md`](./contract/how-this-works.md).
...
Open [`mongodb/how-this-works.md`](./mongodb/how-this-works.md).
...
Open [`mysql/how-this-works.md`](./mysql/how-this-works.md).
...
Open [`postgres/how-this-works.md`](./postgres/how-this-works.md).
...
- start with `dual-db-boundaries.json` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/database/` exists so this slice has one obvious home.
