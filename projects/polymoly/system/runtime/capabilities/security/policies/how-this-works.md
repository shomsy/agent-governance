---
title: system-runtime-capabilities-security-policies-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities Security Policies How This Works

## What this folder is

`system/runtime/capabilities/security/policies/` is one shipped runtime capability slice.
...
- this folder contributes concrete assets or defaults, often starting with `no-public-database-port.yaml`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `no-public-database-port.yaml`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as no-public-database-port.yaml
    participant Next as compose/
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/security/policies/` through the current story
    Owned->>Owned: Step 2: perform the folder-owned work in `no-public-database-port.yaml`
...
- **Step 2:** The first real work starts in `no-public-database-port.yaml`.
- **Step 3:** From here, the story moves to one smaller file, child slice, or boundary that can do the next concrete job.
- **Step 4:** At the end, the caller has something concrete to carry forward: a file on disk, a rendered asset, a proof artifact, or a clear next state.

## Direct files in this folder

### `no-public-database-port.yaml`

This file ships the `no-public-database-port.yaml` config, policy, or data asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/security/policies/` story needs this responsibility, it opens `no-public-database-port.yaml`
...
- the shipped `no-public-database-port.yaml` asset
...
sequenceDiagram
    ...
    participant File as no-public-database-port.yaml
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `no-public-database-port.yaml`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `no-public-database-port.yaml` because this file owns the next small responsibility.
...
Open [`compose/how-this-works.md`](./compose/how-this-works.md).
...
- start with `no-public-database-port.yaml` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/security/policies/` exists so this slice has one obvious home.
