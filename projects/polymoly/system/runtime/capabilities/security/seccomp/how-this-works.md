---
title: system-runtime-capabilities-security-seccomp-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Runtime Capabilities Security Seccomp How This Works

## What this folder is

`system/runtime/capabilities/security/seccomp/` is one shipped runtime capability slice.
...
- this folder contributes concrete assets or defaults, often starting with `default.json`
...
flowchart TD
    ...
    A["1. a higher engine or render flow has already decided this shipped runtime slice matters"]:::step1 --> B["2. this folder contributes concrete assets or defaults, often starting with `default.json`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as default.json
    participant Next as NextStep
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/runtime/capabilities/security/seccomp/` through the current story
    Owned->>Owned: Step 2: perform the folder-owned work in `default.json`
...
- **Step 2:** The first real work starts in `default.json`.
...
This file ships the `default.json` config, policy, or data asset that the next technical step reads directly.
...
- when the `system/runtime/capabilities/security/seccomp/` story needs this responsibility, it opens `default.json`
...
- the shipped `default.json` asset
...
- start with `default.json` when the shipped asset or contract itself looks wrong

## What to remember

- `system/runtime/capabilities/security/seccomp/` exists so this slice has one obvious home.
