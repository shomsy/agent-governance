---
title: system-adapters-env-how-this-works
owner: platform@polymoly
last_reviewed: 2026-03-13
classification: internal
---

# System Adapters Env How This Works

## What this folder is

`system/adapters/env/` owns environment-file generation and environment-specific rendering boundaries.

It is the place where engine decisions become `.env` style outputs or environment-specific settings.
...
- this folder does one small machine-facing job, often starting in `apply_environment_intent.go`
...
flowchart TD
    ...
    A["1. a higher story hands this slice one real machine job"]:::step1 --> B["2. this slice does one concrete machine job, often starting in `apply_environment_intent.go`"]:::step2
...
sequenceDiagram
    ...
    participant Owned as LoadEnvironment
    participant Next as environments/
    participant Result as VisibleResult

    Entry->>Owned: Step 1: reach `system/adapters/env/` through the current story
    Owned->>Owned: Step 2: call `LoadEnvironment(...)` to do the main folder-owned work
...
- **Step 2:** The first real work starts in `apply_environment_intent.go` through `LoadEnvironment(...)`.
- **Step 3:** From here, the story moves to one smaller file, child slice, or boundary that can do the next concrete job.
- **Step 4:** At the end, the caller has something concrete to carry forward: a file on disk, a rendered asset, a proof artifact, or a clear next state.

## Direct files in this folder

### `apply_environment_intent.go`
...
- when the `system/adapters/env/` story needs this responsibility, it opens `apply_environment_intent.go`
...
sequenceDiagram
    ...
    participant File as apply_environment_intent.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `apply_environment_intent.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `apply_environment_intent.go` because this file owns the next small responsibility.
...
### `generate_environment_variables.go`

This file is one direct stop in the story for this folder.

Why this name is honest:

- its main action is still visible in the code, starting with `LoadExample(...)`

When the story opens this file:

- when the `system/adapters/env/` story needs this responsibility, it opens `generate_environment_variables.go`
...
- the result of `LoadExample(...)` for the next caller
- a concrete return value, file write, check result, or summary depending on the path

Why you open it first:

- open this file when the symptom points to `LoadExample(...)` doing the wrong thing

```mermaid
sequenceDiagram
    autonumber
    participant Caller as ParentFlow
    participant File as generate_environment_variables.go
    participant Next as NextStep

    Note over Caller,File: Input: parent flow values, repo state, or shipped asset context
    Caller->>File: Step 1: enter `generate_environment_variables.go`
    File->>File: Step 2: do the file-owned work
    File->>Next: Step 3: hand the concrete result forward
```

- **Step 1:** The story reaches `generate_environment_variables.go` because this file owns the next small responsibility.
- **Step 2:** The file does its own narrow action instead of mixing it into a bigger caller.
- **Step 3:** The next caller gets a concrete result, not another vague promise.
...
Open [`environments/how-this-works.md`](./environments/how-this-works.md).
...
- start with `LoadEnvironment(...)` in `apply_environment_intent.go` when that action looks wrong
- start with `LoadExample(...)` in `generate_environment_variables.go` when that action looks wrong

## What to remember

- `system/adapters/env/` exists so this slice has one obvious home.
